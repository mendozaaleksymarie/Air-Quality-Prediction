/*
 * MILES DATA STATION - ATTEMPT 11.2 (ROBUST PMS7003 SYNC)
 * Updates: 5s Reading Interval + Header Sync + Checksum Validation
 */
...
// Robust PMS7003 reader: header sync + frame length + checksum.
...
void setup() {
    ...
    int fileNum = 1;
    while (SD.exists("/MILES_D" + String(fileNum) + ".csv")) fileNum++;
    fileName = "/MILES_D" + String(fileNum) + ".csv";

    File file = SD.open(fileName, FILE_WRITE);
    if (file) {
        file.println("Time,PM2.5,PM10,Temp,Hum,Gas,CO,Class,Remark");
        file.close();
    }
    ...
    lcd.clear(); lcd.setCursor(0, 0); lcd.print("SENSORS WARMING UP");
    for (int i = 120; i > 0; i--) {
        lcd.setCursor(0, 2); lcd.print("Time Left: "); lcd.print(i); lcd.print("s ");
        delay(1000);
    }
...
void loop() {
    ...
    if (now - lastRead >= 5000) {
        ...
        Blynk.virtualWrite(V0, data.pm2_5); Blynk.virtualWrite(V1, data.pm10);
        Blynk.virtualWrite(V2, data.temp); Blynk.virtualWrite(V3, data.hum);
        Blynk.virtualWrite(V4, data.gas); Blynk.virtualWrite(V5, data.co);
        Blynk.virtualWrite(V7, blynkFullRemark);

        File file = SD.open(fileName, FILE_APPEND);
        if (file) {
            file.printf("%s,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%d,%s\n",
                        getTimeString().c_str(), data.pm2_5, data.pm10, data.temp, data.hum, data.gas, data.co, lastClass, lcdRemark.c_str());
            file.close();
        }
