...
for csv_file in csv_files:
    ...
    try:
        ...
        if "MQ2_ADC" in df.columns and "MQ7_ADC" in df.columns:
            # Convert ADC to PPM in-place
            df["MQ2_ADC"] = df["MQ2_ADC"].apply(lambda x: adc_to_ppm(x, MQ2_OFFSET, 1000.0))
            df["MQ7_ADC"] = df["MQ7_ADC"].apply(lambda x: adc_to_ppm(x, MQ7_OFFSET, 100.0))
...
print("\n✅ All CSV files converted from ADC to PPM!")
