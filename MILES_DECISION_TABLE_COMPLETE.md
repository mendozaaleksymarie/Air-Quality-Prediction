...
- **Workers see the REASON** - not a generic status, but a specific cause + action (e.g., "HAZARDOUS: DUST AND COMBUSTIBLE GAS CRITICAL, PREPARE EVACUATION")
...
IF Tw 27-30°C:
   ...
   → Add CAUTION remark: "ELEVATED HEAT, SLOW WORK AND HYDRATE"
   
IF Tw > 30°C:
   → Escalate "Safe" → "Caution"
   → Escalate "Caution" → "Hazardous"
   → Keep "Hazardous" → "Hazardous"
   → Add HAZARDOUS remark: "HIGH WET-BULB TEMP, STOP NON-ESSENTIAL PHYSICAL WORK"
...
|---|---|---|---|---|---|
| 25°C | 40% | 14°C | Safe | **Safe** | SAFE: ALL SENSORS NORMAL, CONTINUE OPERATIONS |
| 25°C | 40% | 14°C | Caution | **Caution** | CAUTION: FINE DUST RISING, REDUCE DUST EXPOSURE NOW |
| 28°C | 65% | 23°C | Safe | **Safe** | SAFE: ALL SENSORS NORMAL, CONTINUE OPERATIONS |
| 28°C | 65% | 23°C | Caution | **Caution** | CAUTION: FINE DUST RISING, REDUCE DUST EXPOSURE NOW |
| 32°C | 60% | 27°C | Safe | **Caution** | CAUTION: ELEVATED HEAT, SLOW WORK AND HYDRATE |
| 32°C | 60% | 27°C | Caution | **Hazardous** | HAZARDOUS: HIGH WET-BULB TEMP, STOP NON-ESSENTIAL PHYSICAL WORK |
| 35°C | 70% | 30.5°C | Safe | **Hazardous** | HAZARDOUS: HIGH WET-BULB TEMP, STOP NON-ESSENTIAL PHYSICAL WORK |
| 35°C | 70% | 30.5°C | Caution | **Hazardous** | HAZARDOUS: HIGH WET-BULB TEMP, STOP NON-ESSENTIAL PHYSICAL WORK |
| 38°C | 75% | 34°C | Any | **Hazardous** | HAZARDOUS: EXTREME HEAT DETECTED, EVACUATE TO COOLING AREA NOW |
| 40°C | 80% | 36°C | Any | **Hazardous** | HAZARDOUS: EXTREME HEAT DETECTED, EVACUATE TO COOLING AREA NOW |
...
Define Caution Thresholds:
   ...
   PM10_Cau = (pm10 >= 151)
   Gas_Cau = (gas >= 40)
   CO_Cau = (co >= 10 AND co <= 30)

Count_Hazard = PM2.5_Haz + PM10_Haz + Gas_Haz + CO_Haz
Count_Caution = PM2.5_Cau + PM10_Cau + Gas_Cau + CO_Cau

IF (Count_Hazard >= 1):
   → FORCE CLASS 2 (HAZARDOUS)
   
ELSE IF (Count_Caution >= 1) AND (cls from model == 0):
...
| 7: High Humidity | 673 | None | Normal PM/Gas + elevated humidity | 0 | SAFE: ELEVATED HUMIDITY ONLY, CONTINUE TASKS WITH HYDRATION | Humidity context |
   → Remark: "HAZARDOUS: EVACUATE TO COOLING AREA NOW"
   → STOP - Override all sensor logic
```
...
IF (Count_Hazard >= 1) OR (Tw > 30°C AND cls from model indicates hazard):
   → FORCE CLASS 2 (HAZARDOUS)
...
IF Humidity >= 95% AND Gas <= 40 ppm AND Tw <= 35°C:
   → FORCE CLASS 0 (SAFE)
   → Remark: "SAFE: CONTINUE WORK, NO EVACUATION"
   → STOP - Misting is NOT a health hazard
```
...
**UPDATED:** Synchronized with firmware implementation (esp32_embedded_ml.ino v11.5)  
