...
# Conversion formulas (inverse of: PPM = ((ADC / 4095.0) × scale))
def ppm_to_adc_mq2(ppm):
    return (ppm * 4095.0) / 1000.0

def ppm_to_adc_mq7(ppm):
    return (ppm * 4095.0) / 100.0

print("Starting PPM to ADC conversion process (without offsets)...")
print("MQ2_ADC = (MQ2_PPM × 4095.0) / 1000.0")
print("MQ7_ADC = (MQ7_PPM × 4095.0) / 100.0\n")
...
for file_path in sorted(data_files):
    try:
        ...
        if 'MQ2_PPM' in df.columns and 'MQ7_PPM' in df.columns:
            # Convert PPM to ADC
            df['MQ2_ADC'] = ppm_to_adc_mq2(df['MQ2_PPM']).round(2)
            df['MQ7_ADC'] = ppm_to_adc_mq7(df['MQ7_PPM']).round(2)
            
            # Drop the old PPM columns
            ...
            print(f"⚠️  {file_path.name}: Missing MQ2_PPM or MQ7_PPM columns")
...
try:
    ...
    # Convert PPM to ADC
    df_combined['MQ2_ADC'] = ppm_to_adc_mq2(df_combined['MQ2_PPM']).round(2)
    df_combined['MQ7_ADC'] = ppm_to_adc_mq7(df_combined['MQ7_PPM']).round(2)
    
    # Drop the old PPM columns
    df_combined = df_combined.drop(columns=['MQ2_PPM', 'MQ7_PPM'])
    
    # Use converted ADC values for recalculation
    ...
    # Is gas accelerating (ADC-based threshold, no offsets)
...
try:
    ...
    # Convert PPM to ADC
    df_dataset['MQ2_ADC'] = ppm_to_adc_mq2(df_dataset['MQ2_PPM']).round(2)
    df_dataset['MQ7_ADC'] = ppm_to_adc_mq7(df_dataset['MQ7_PPM']).round(2)
    
    # Drop the old PPM columns
    df_dataset = df_dataset.drop(columns=['MQ2_PPM', 'MQ7_PPM'])
    ...
    print(f"   MQ2_ADC range: {df_dataset['MQ2_ADC'].min():.2f} - {df_dataset['MQ2_ADC'].max():.2f}")
    print(f"   MQ7_ADC range: {df_dataset['MQ7_ADC'].min():.2f} - {df_dataset['MQ7_ADC'].max():.2f}")
    
except Exception as e:
    print(f"❌ combined_dataset.csv: Error - {str(e)}")

print("\n" + "="*60)
print("✅ PPM to ADC conversion complete!")
print("="*60)
